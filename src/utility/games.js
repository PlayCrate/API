...
exports.gameInfo = async (gameID) => {
  const { data: gameInfo } = await fetch(
...
exports.gameVotesInfo = async (gameID) => {
  const { data: votesInfo } = await fetch(
    `https://games.roblox.com/v1/games/votes?universeIds=${gameID}`,
    {
      method: "GET",
    }
  ).then((res) => res.json());
  ...
  const ratings =
    upVotes + downVotes === 0 ? 0 : (upVotes / (upVotes + downVotes)) * 100;
